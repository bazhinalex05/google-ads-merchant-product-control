...
function readSettings_(ss) {
  var defaults = {
    ...
    benchmarkLabelField: "product_type_l1",
...
function writeSettingsTemplate_(sheet, settings) {
  var rows = [
    ...
    ["-- 2. Фільтри фідів і Merchant API --", "", ""],
    ...
    ["-- 5. Сезонність --", "", ""],
    ["active_season_winter", settings.activeSeasonWinter, "Увімкніть, щоб дозволити товари з позначеною зимою, зокрема примусово продовжити їх сезон незалежно від календаря. Діє лише з увімкненим фільтром сезонності; інші обмеження залишаються."],
    ["active_season_spring", settings.activeSeasonSpring, "Увімкніть, щоб дозволити товари з позначеною весною, зокрема примусово продовжити їх сезон незалежно від календаря. Діє лише з увімкненим фільтром сезонності; інші обмеження залишаються."],
    ...
    ["product_type_id_prefixes_to_strip", settings.productTypeIdPrefixesToStrip, "Лише якщо категорії беруться із зовнішнього XML і повний ID не знайдено. Приклад: у Merchant UA123, у XML 123 — вкажіть UA. Скрипт відкине префікс з ID Merchant для пошуку product_type у XML. Кілька префіксів — через кому. ID у Products не змінюються, статистика не об'єднується. Інакше залиште порожнім."],
    ["product_type_id_suffixes_to_strip", settings.productTypeIdSuffixesToStrip, "Лише якщо категорії беруться із зовнішнього XML і повний ID не знайдено. Приклад: у Merchant 123_ru, у XML 123 — вкажіть _ru. Скрипт відкине постфікс з кінця ID Merchant для пошуку product_type у XML. Кілька постфіксів — через кому. ID у Products не змінюються, статистика не об'єднується. Інакше залиште порожнім."],
    ["-- 7. Карантин --", "", ""],
    ["-- 7.1 Значення для допфіда --", "", ""],
    ...
    ["enable_product_diagnostics", settings.enableProductDiagnostics, "false = не записувати ProductDiagnostics для дуже великих каталогів. При цьому в ProductDiagnostics усе одно записуються три поля: ID товару, чи перебуває він у карантині, дата останнього виходу з карантину. Вони зберігаються тут, щоб наступний запуск не враховував витрати до виходу з карантину."],
    ["enable_dashboard_data", settings.enableDashboardData, "false = не записувати DashboardData для дуже великих каталогів. Наявні дані залишаються, але не оновлюються; пов'язаний з ними Dashboard може показувати попередній результат."],
    ["enable_dashboard", settings.enableDashboard, "false = не будувати Dashboard і графіки для дуже великих каталогів. Наявний Dashboard не видаляється; його формули можуть перераховуватися при оновленні джерел даних."],
    ["dashboard_sheet_name", settings.dashboardSheetName, "Назва візуального листа Dashboard. Скрипт його не перезаписує. Уточнення: ручне оформлення зберігається, але порожній або розпізнаний автоматичний Dashboard скрипт будує заново."],
    ["dashboard_data_sheet_name", settings.dashboardDataSheetName, "Службовий лист з даними для Dashboard. Скрипт повністю перезаписує тільки його. Йдеться про пару Dashboard і DashboardData: інші автоматичні листи також можуть оновлюватися. Дані цього листа оновлюються, якщо enable_dashboard_data увімкнено."],
    ["priority_sheet_name", settings.prioritySheetName, "Лист Priorities для ручних priority group від клієнта. Скрипт створює тільки заголовки і не перезаписує ручний список. Ручні колонки: id, priority_group, comment; скрипт також оформлює їх, якщо форматування увімкнено."],
    ["enable_dashboard_from_diagnostics", settings.enableDashboardFromDiagnostics, "true = будувати Dashboard з готового ProductDiagnostics без читання Merchant API. Це окремий режим лише для Dashboard та DashboardData: звичайний розрахунок товарів і карантинів не виконується. Потрібна вже заповнена діагностика; дані успадковують її актуальність."],
    ["enable_managed_sheet_formatting", settings.enableManagedSheetFormatting, "false = писати тільки значення і не запускати масове форматування службових листів. Не вимикає все оформлення: Settings, ProductTypes і Seasonality продовжують форматуватися; Dashboard має окрему логіку оформлення."],
    ["enable_previous_state_read", settings.enablePreviousStateRead, "false = не читати старий стан ProductDiagnostics/Products перед побудовою рядків. Виняток: ID товару, ознака карантину та дата останнього виходу з ProductDiagnostics читаються завжди, щоб визначити вихід і не враховувати старі витрати."],
    ["enable_sheet_protection", settings.enableSheetProtection, "false = не оновлювати захист службових листів. Не знімає раніше встановлений захист інших листів. Захист Seasonality скрипт знімає окремо незалежно від цієї галочки."],
    ["enable_products_write", settings.enableProductsWrite, "false = не перезаписувати Products; корисно для окремого запуску діагностики або Dashboard. Наявний допфід залишається старим. У звичайному режимі порожній Products спричинить помилку; інші ввімкнені розрахунки та записи продовжуються."],
    ["write_chunk_size", settings.writeChunkSize, "Кількість рядків в одному setValues-блоці для великих листів, напр. 3000-10000. Застосовується до запису Products і повної ProductDiagnostics; це не розмір сторінки Merchant API."],
    ["product_diagnostics_start_row", settings.productDiagnosticsStartRow, "1 = писати ProductDiagnostics з початку. 110001 = дописати хвіст з цього рядка даних. Це номер рядка даних без заголовка, не продовження всього скрипта. Розрахунки виконуються заново; попередні рядки повної діагностики не оновлюються, крім трьох полів карантину. Не використовуйте для звичайного запуску."],
