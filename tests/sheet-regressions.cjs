...
const external = { '123': { productType: 'Base' }, '123_ua': { productType: 'Exact' } };
assert.equal(ctx.findExternalProductTypeMatch_('123_ua', external, [], ['_ua']).productType, 'Exact');
assert.equal(ctx.findExternalProductTypeMatch_('123_copy', external, [], ['_copy']).matchId, '123');
assert.equal(ctx.findExternalProductTypeMatch_('FX123_COPY', external, ['FX'], ['_copy']).matchId, '123');
